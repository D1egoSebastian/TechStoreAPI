...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await register({ Name: name, Email: email, PasswordHash: password });
      const data = await res.json();
      
      if (res.ok) {
        // Auto-login after register
        const loginRes = await login({ Email: email, PasswordHash: password });
        if (loginRes.ok) {
      ...
      <div className="max-w-md w-full">
        ...
        <div className="bg-[#1a1a1a] rounded-3xl shadow-xl p-8 border border-white/10">
          ...
          <p className="text-gray-400 text-center mb-8">
            Únete a TechStore y compra tecnología
          </p>
